...
const Introduce9Lesson22 = () => {
  ...
  const markLessonComplete = () => {
    ...
    if (!completed.includes("3-introduce-9-lesson-22")) {
      completed.push("3-introduce-9-lesson-22");
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-indigo-50 to-white p-4 font-fredoka overflow-x-hidden">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          <Button variant="outline" size="icon" onClick={() => navigate("/activities/module-3?last=3-introduce-9-lesson-22")} className="rounded-full border-2 border-white bg-white/50 backdrop-blur-sm">
            ...
            {currentStep === 'complete' && (
              <Card className="bg-gradient-to-br from-purple-600 via-indigo-600 to-blue-600 shadow-2xl rounded-[4rem] overflow-hidden p-16 text-center text-white space-y-10 animate-in zoom-in-95 duration-700">
                ...
                <div className="flex gap-4 w-full pt-8">
                  ...
                  <Button onClick={() => navigate("/activities/module-3?last=3-introduce-9-lesson-22")} className="h-24 flex-1 bg-white text-purple-600 hover:bg-rose-50 text-3xl rounded-[2rem] shadow-2xl">
                    Yay! ✨
                  </Button>
                </div>
              </Card>
              ...
              <div className="fixed bottom-6 right-6 z-[100] animate-in slide-in-from-right-4 fade-in duration-300">
                <Card className={`flex items-center gap-4 px-6 py-4 shadow-2xl rounded-2xl border-4 ${showFeedback === 'correct' ? 'bg-green-50 border-green-400' : 'bg-red-50 border-red-400'}`}>
                  ...
                  <h4 className={`text-2xl font-fredoka ${showFeedback === 'correct' ? 'text-green-700' : 'text-red-700'}`}>
                    {showFeedback === 'correct' ? '9!' : 'Try Again!'}
                  </h4>
